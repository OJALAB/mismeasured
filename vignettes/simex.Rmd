...
The improved method (Sevilimedu & Yu, 2026) computes an exact fixed-matrix
correction instead of extrapolation. It needs only $B = 1$ replicate and is
the default for a single misclassified covariate. For binary covariates the
...
- **Single misclassified factor.** `mc(z, Pi)`. Default is the improved
  ...
  Kuechenhoff et al. extrapolation if your downstream tooling expects it.
