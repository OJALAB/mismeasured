# SIMEX and MC-SIMEX with mismeasured

## Scope

This vignette covers the simulation-extrapolation family of corrections
in **mismeasured**: classical SIMEX for continuous measurement error
...
$`U \sim N(0, \sigma_u^2)`$, regression coefficients are biased toward
zero (attenuation bias). SIMEX corrects this in two steps:

1.  **Simulation.** For $`\lambda \in \{0.5, 1, 1.5, 2\}`$ and
    $`b = 1, \ldots, B`$, generate
    $`W^*_{b,\lambda} = W + \sqrt{\lambda} \cdot U_b`$ with
    $`U_b \sim N(0, \sigma_u^2)`$. Refit the model on each
    pseudo-dataset.

2.  **Extrapolation.** Fit a polynomial (quadratic, linear, or
    loglinear) to $`\bar{\hat\beta}(\lambda)`$ as a function of
    $`\lambda`$ and extrapolate to $`\lambda = -1`$, the value implying
...
raised to fractional powers. The improved method computes a closed-form
correction factor $`c(\lambda)`$ in place of the parametric
extrapolation.
...
![](simex_files/figure-html/me-plot-1.png)![](simex_files/figure-html/me-plot-2.png)
...
#> Residuals:
#>      Min       1Q   Median       3Q      Max 
#> -5.04944 -0.79086  0.06307  0.86372  5.21285 
#> 
#> Naive coefficients:
#>         z11         z21 (Intercept) 
#>      1.5543      1.2123      1.3934 
#> 
...
When the outcome itself is misclassified, use
...
- `simex_sim_cpp` — continuous ME with IRLS fitting plus model-vcov
  accumulation
- `mcsimex_sim_cpp` — single-variable MC-SIMEX
- `mcsimex_multi_sim_cpp` — multi-variable MC-SIMEX with optional
  response `mc`

The IRLS solver supports Gaussian, Poisson, and Binomial canonical
links. Matrix exponentiation for $`\Pi^\lambda`$ uses
eigendecomposition.
...
- **Continuous additive error, known $`\sigma_u`$.** `me(x, sd)`.
  Quadratic extrapolation is the default; use `extrapolation = "linear"`
...
estimation in parametric measurement error models. *J. Amer. Statist.
Assoc.* 89(428), 1314–1328.
...
measurement error jackknife. *J. Amer. Statist. Assoc.* 90(432),
1247–1256.
